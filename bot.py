...
async def handle_tg(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    ...
    # Initialize chat history for resume
    ...
    # Hide reply keyboard after entering chat mode
    await message.answer(
...
async def on_chat_message(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    ...
    thinking_msg = await message.answer("Сейчас прикину, что тебе продать… (думаю 1-2 секунды)")
    try:
        ...
        await thinking_msg.edit_text("Не пошло. Попробуй чуть позже или коротко переформулируй.")
        return

    await thinking_msg.edit_text(reply or "Ладно. Продолжай, я рядом.")
...
async def on_poehaly(callback: CallbackQuery, state: FSMContext, openai: AsyncOpenAI) -> None:
    ...
    try:
        resume_text = await gpt_resume(openai, model=model, system_prompt=resume_system_prompt, payload=payload)
    except Exception:
        log.exception("GPT resume error")
        await msg.edit_text("Не смог оформить резюме. Попробуйте ещё раз.")
        return
    ...
    await callback.message.answer("Готово. Резюме отправлено менеджеру в канал.")
    await state.clear()
...
async def main() -> None:
    ...
    async def tg_handler(m: Message, s: FSMContext) -> None:
        await handle_tg(m, s, openai)
    ...
    # Chat mode
    async def chat_handler(m: Message, s: FSMContext) -> None:
        await on_chat_message(m, s, openai)

    dp.message.register(chat_handler, UserFlow.CHAT)

    # Support escalation
    async def support_to_admin_handler(c: CallbackQuery, s: FSMContext) -> None:
        await on_support_to_admin(c, s, openai)

    dp.callback_query.register(support_to_admin_handler, F.data == "support:to_admin")

    # Poehaly button
    async def poehaly_handler(c: CallbackQuery, s: FSMContext) -> None:
        await on_poehaly(c, s, openai)
