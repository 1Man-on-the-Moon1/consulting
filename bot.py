...
def make_docx_bytes(title: str, markdown_table: str) -> bytes:
    ...
    # Put table as text (simple + robust). If you want a real Word table later, we'll parse markdown.
    doc.add_paragraph(markdown_table)
    buf = io.BytesIO()
    doc.save(buf)
    return buf.getvalue()
...
def make_pdf_bytes(title: str, text: str) -> bytes:
    # Basic PDF with monospaced-ish font fallback.
    buf = io.BytesIO()
...
async def on_user_document(message: Message, state: FSMContext) -> None:
    ...
    title, md_table = json_table_to_markdown(table)
    md_table = _truncate(md_table, 30000)

    docx_bytes = await asyncio.to_thread(make_docx_bytes, title, md_table)
    pdf_bytes = await asyncio.to_thread(make_pdf_bytes, title, md_table)
