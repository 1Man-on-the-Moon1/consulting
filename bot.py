...
async def on_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "Здравствуй, клиент.\n"
        "Нажмешь кнопку — начнем по-человечески: по анкете соберу данные и передам менеджеру.\n"
        "Поехали? Жми `HELLS BELLS`.",
...
async def handle_name(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    text = (message.text or "").strip()
    if should_offer_support(text):
        # Пытаемся ответить GPT с первого раза; если не получится — даём кнопку ПОДДЕРЖКА.
        await state.update_data(support_question=text)
        reply = ""
        try:
            reply = await gpt_chat_reply(
                openai,
            ...
            log.exception("GPT attempt failed in GET_NAME")

        if not (reply or "").strip():
            await message.answer(
                "Клиент, тут без угадываний. Свяжу с поддержкой.\nНажми кнопку.",
                reply_markup=support_inline_keyboard(),
            )

        await message.answer("ХЕЛЛС БЕЛЛС.\nКак тебя зовут, клиент? Не тяни резину.")
        return
...
async def handle_age(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    text = (message.text or "").strip()
    if should_offer_support(text):
        await state.update_data(support_question=text)
        reply = ""
        try:
            ...
            log.exception("GPT attempt failed in GET_AGE")
        ...
        await message.answer("Сколько тебе лет? Только без сюрпризов.")
        return
...
async def handle_experience(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    text = (message.text or "").strip()
    if should_offer_support(text):
        await state.update_data(support_question=text)
        reply = ""
        try:
            ...
            log.exception("GPT attempt failed in GET_EXPERIENCE")
        ...
        await message.answer("Есть опыт с нейросетями? Сколько и что делал/а. По-честному.")
        return
...
async def handle_goal(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    text = (message.text or "").strip()
    if should_offer_support(text):
        await state.update_data(support_question=text)
        reply = ""
        try:
            ...
            log.exception("GPT attempt failed in GET_GOAL")
        ...
        await message.answer("Чему хочешь научиться в первую очередь? Внятно.")
        return
...
async def handle_phone(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    text = (message.text or "").strip()
    if should_offer_support(text):
        await state.update_data(support_question=text)
        reply = ""
        try:
            ...
            log.exception("GPT attempt failed in GET_PHONE")
        ...
        await message.answer("Оставь телефон для связи (можно WhatsApp).")
        return
    phone = text
...
async def handle_tg(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    try:
        ...
        if should_offer_support(message.text or ""):
            ...
            await state.update_data(support_question=q)
            reply = ""
            try:
                reply = await gpt_chat_reply(
                    openai,
                ...
                log.exception("GPT attempt failed in GET_TG")
            ...
            await message.answer("И Telegram-аккаунт: пришли `@username`. Ну давай.")
            return
...
async def on_chat_message(message: Message, state: FSMContext, openai: AsyncOpenAI) -> None:
    try:
        ...
        log.info(
            "CHAT handler: user_id=%s chat_id=%s state=%s text=%r",
        ...
        try:
            ...
            log.exception("GPT chat error (first attempt)")
            # Retry once; if it still fails -> offer support.
            try:
                await message.bot.send_chat_action(chat_id=message.chat.id, action="typing")
                reply = await gpt_chat_reply(
                    openai,
                ...
                await state.update_data(support_question=text)
                await message.answer(
                    "Клиент, с этим вопросом без угадываний. Свяжу с поддержкой.\nНажми `ПОДДЕРЖКА`.",
                    reply_markup=support_inline_keyboard(),
                )
                return
        ...
        log.exception("CHAT handler fatal error")
        await state.update_data(support_question=(message.text or "").strip())
        await message.answer(
            "Клиент, что-то пошло не так. Свяжу с поддержкой.\nНажми `ПОДДЕРЖКА`.",
            reply_markup=support_inline_keyboard(),
        )
...
async def on_support_to_admin(callback: CallbackQuery, state: FSMContext, openai: AsyncOpenAI) -> None:
    ...
    support_reply_text = (
        ...
        "Больше пока ничего нажимать не надо."
    )
...
async def main() -> None:
    ...
    async def name_handler(message: Message, state: FSMContext) -> None:
        await handle_name(message, state, openai)

    async def age_handler(message: Message, state: FSMContext) -> None:
        await handle_age(message, state, openai)

    async def experience_handler(message: Message, state: FSMContext) -> None:
        await handle_experience(message, state, openai)

    async def goal_handler(message: Message, state: FSMContext) -> None:
        await handle_goal(message, state, openai)

    async def phone_handler(message: Message, state: FSMContext) -> None:
        await handle_phone(message, state, openai)

    dp.message.register(name_handler, UserFlow.GET_NAME)
    dp.message.register(age_handler, UserFlow.GET_AGE)
    dp.message.register(experience_handler, UserFlow.GET_EXPERIENCE)
    dp.message.register(goal_handler, UserFlow.GET_GOAL)
    dp.message.register(phone_handler, UserFlow.GET_PHONE)
