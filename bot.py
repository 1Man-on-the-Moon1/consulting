...
def _try_register_cyrillic_font() -> str:
    """
    ReportLab built-in fonts don't support Cyrillic -> squares.
    On Railway/Linux, DejaVuSans is usually available. Try to register it.
    """
    candidates = [
...
def normalize_table(table: dict) -> tuple[str, list[str], list[list[str]]]:
    ...
    # Ensure numbering column exists and row widths match
    if columns[0] != "№ п/п":
        columns = ["№ п/п"] + columns
        norm_rows = [[str(i + 1)] + row for i, row in enumerate(norm_rows)]
    else:
...
def make_pdf_table_bytes(title: str, columns: list[str], rows: list[list[str]]) -> bytes:
    font_name = _try_register_cyrillic_font()
    ...
    # Use Paragraph to wrap long text inside cells
    data = [[Paragraph(str(c), styleN) for c in columns]]
    for r in rows:
        data.append([Paragraph(str(cell), styleN) for cell in r])

    # Column widths similar to screenshot: narrow number, wide name, medium status, wide link
    col_widths = [30, 240, 120, 140]
    if len(columns) != 4:
        # fallback: split available width evenly
        total = A4[0] - doc.leftMargin - doc.rightMargin
        col_widths = [total / len(columns)] * len(columns)
...
async def gpt_table_json(client: AsyncOpenAI, model: str, system_prompt: str, payload: str) -> dict:
    ...
    instruction = (
        ...
        "Формат:\n"
        '{ "title": "...", "columns": ["..."], "rows": [["...","..."],["...","..."]] }\n'
        "В ячейках короткий текст. Если данных мало — заполни тем, что есть, без выдумок."
    )
    messages = [
...
async def on_user_document(message: Message, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    auction_text = _truncate(str(data.get("auction_text") or ""), MAX_DOC_CHARS)
    appendix_text = _truncate(str(data.get("appendix_text") or ""), MAX_DOC_CHARS)
    manual_text = _truncate(str(data.get("manual_text") or ""), MAX_DOC_CHARS)
    ...
    try:
        ...
        table = await gpt_table_json(OPENAI, model, system_prompt, payload)
    except Exception:
    ...
    pdf_bytes = await asyncio.to_thread(make_pdf_table_bytes, title, columns, rows)
    ...
    await message.answer_document(
        BufferedInputFile(pdf_bytes, filename="table.pdf"),
        caption="Таблица соответствия (PDF).",
    )
